...
class FinesService
{
    ...
    public function getFinesData(array $filters = [], $count = false)
    {
        ...
        return array_map(function (SafoPenalty $fine) {
            return [
                ...
                'cu' => [
                    'type' => (is_null($fine->getCustomer()) ? 0 : ($fine->getCustomer()->getMaintainer() ? 1 : ($fine->getCustomer()->getGoldList() ? 2 : ($fine->getCustomer()->getSilverList() ? 3 : 0))))
                ]
            ];
        }, $fines);
    }
    ...
    public function createExtraPayment(SafoPenalty $fine, Penalty $penalty, $transaction) {
        ...
        $extra_payment = new ExtraPayments(
                $fine->getCustomer(), $fleet_modena, $transaction, $penalty->getAmount(), $penalty->getType() == 'penalties' ? "penalty" : "extra", $reasonsAmounts, true
        );
